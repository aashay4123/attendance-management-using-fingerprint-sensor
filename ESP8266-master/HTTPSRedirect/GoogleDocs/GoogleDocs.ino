...
const char *GScriptId = "AKfycbwZ8mhcw5xwknuuY9tRXeUu2y2hE2ZNMliE07zDDvLfOoqbE4Y";

const int httpsPort = 443;

// echo | openssl s_client -connect script.google.com:443 |& openssl x509 -fingerprint -noout
const char* fingerprint = "AD 33 25 D3 16 2F 30 9F C6 B9 DB DA A4 51 72 6E FD AD 3D AD";
//const uint8_t fingerprint[20] = {};

// Write to Google Spreadsheet
String url1 = String("/macros/s/") + GScriptId + "/exec?value=2";
...
void setup() {
  ...
  */
  ...
  // Send memory url1 to Google Sheets
  payload = payload_base + "\"" + free_heap_before + "," + free_stack_before + "\"}";

  client->POST(url1, host, payload, false);
  ...
  Serial.println("\nGET: Fetch Google Calendar url1:");
  Serial.println("================================");
...
void loop() {
  ...
  Serial.println("GET url1 from cell 'A1':");
  ...
  Serial.println("POST append memory url1 to spreadsheet:");
  payload = payload_base + "\"" + ESP.getFreeHeap() + "," + ESP.getFreeContStack() + "\"}";
  if (client->POST(url1, host, payload)) {
    ;
  }
  else {
    ++error_count;
